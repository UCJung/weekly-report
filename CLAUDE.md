...
#### 파이프라인 트리거 조건
사용자 요청 앞에 `[]` 태그가 있으면 **반드시 파이프라인을 실행**한다.
- 예: `[추가기능]`, `[오류수정]`, `[작업실행]`, `[기능개선]`, `[기능수정]`, `[버그수정]`, `[테스트데이터 생성]` 등
- `[]` 태그가 없는 일반 요청은 파이프라인 없이 직접 처리한다

#### `[]` 태그 요청 시 WORK 배정 프로세스 (필수)
1. **`tasks/multi-tasks/WORK-LIST.md`를 읽어** 현재 미완료(IN_PROGRESS) WORK가 있는지 확인한다
2. 미완료 WORK가 있으면 사용자에게 질문한다:
   > "현재 진행 중인 WORK-XX ({제목})가 있습니다. 이 WORK에 추가 TASK로 진행할까요, 아니면 새 WORK를 생성할까요?"
3. 미완료 WORK가 없으면 자동으로 새 WORK를 생성한다
4. 사용자 응답에 따라:
   - **기존 WORK에 추가** → 3.2 절차 (TASK MD 생성 → builder → verifier → committer)
   - **새 WORK 생성** → 3.1 전체 파이프라인 (planner → scheduler → builder → verifier → committer)
5. **`[WORK 시작]` 태그**는 항상 새 WORK를 생성한다 (질문 없이 즉시 planner 시작)
...
- planner: Sequential Thinking으로 요구사항 분석 → `tasks/multi-tasks/WORK-XX/PLAN.md` 작성
  - **PLAN.md 최상단에 `## 요청사항` 섹션을 반드시 포함**한다
  - 사용자가 직접 요청한 경우: 요청 문구를 **원문 그대로** 기재
  - `tasks/Require/Require-NN.md` 파일로 파이프라인을 시작한 경우: 해당 파일 경로를 기재 (예: `tasks/Require/Require-06.md`)
- scheduler: PLAN.md 기반으로 TASK 분해 + 진행 관리
  - TASK MD 생성 → `tasks/multi-tasks/WORK-XX/WORK-XX-TASK-YY.md` (**3-Step 형식 필수**, 아래 3.5 참조)
  - **PROGRESS.md 생성** → `tasks/multi-tasks/WORK-XX/PROGRESS.md` (아래 3.6 참조)
...
### 3.2 WORK 내 추가 작업 (계획 완료 후 추가 요청)

기존 WORK의 계획된 TASK가 모두 완료된 후, 사용자가 추가 작업이나 오류 수정을 요청하면:
1. **TASK MD 생성**: `WORK-XX-TASK-YY.md` 작성 (3-Step 형식, 상단에 `## 요청사항` 섹션 필수 포함)
2. **즉시 실행**: builder → verifier → committer 순서로 진행 (planner/scheduler 단계 생략)
3. **PROGRESS.md 업데이트**: 추가 TASK를 테이블에 추가하고 상태를 기록

```
추가 요청 분석
  → TASK MD 생성 (## 요청사항 + 3-Step 구조)
  → builder (코드 구현) → verifier (검증) → committer (보고서 + 커밋)
  → PROGRESS.md 업데이트
```

### 3.3 WORK-LIST.md 관리 규칙

`tasks/multi-tasks/WORK-LIST.md`는 모든 WORK의 마스터 목록이다.

| 상태 | 의미 |
|------|------|
| COMPLETED | 모든 TASK 완료 + push 완료 |
| IN_PROGRESS | TASK 진행 중 (아직 push 안 됨) |

#### 갱신 시점
- **WORK 생성 시**: WORK-LIST.md에 새 행 추가 (상태: `IN_PROGRESS`)
- **git push 시**: 현재 IN_PROGRESS인 WORK를 `COMPLETED`로 갱신, 최종 갱신 날짜 업데이트
- push 커밋에 WORK-LIST.md 변경을 포함한다

#### 절대 금지
- WORK-LIST.md 없이 WORK 디렉터리만 생성하는 것
- push 후에도 IN_PROGRESS 상태를 방치하는 것
...
### 3.5 TASK MD 작성 형식 (3-Step 구조)

scheduler가 TASK MD를 생성할 때 반드시 아래 3-Step 구조를 따른다.
이 형식은 사람이 작업 전 계획을 검토하고, 작업 중 체크리스트로 진행도를 추적하며, 작업 후 검증 명령을 실행하여 결과를 확인할 수 있도록 설계되었다.

**추가 작업(3.2)으로 생성하는 TASK MD**에는 메타 정보 바로 아래에 `## 요청사항` 섹션을 추가하여 사용자의 원래 요청 문구를 그대로 기재한다.

참조 원본: `tasks/TASK-00.md` ~ `tasks/TASK-04.md` (초기 TASK 계획서)

```markdown
# WORK-XX-TASK-YY: {TASK 제목}

> **Phase:** {Phase 번호}
> **선행 TASK:** {의존 TASK 또는 "없음"}
> **목표:** {1~2줄 목표 요약}

## 요청사항
(사용자 요청 원문 — 추가 작업 시 필수, WORK 파이프라인 초기 TASK에서는 PLAN.md에 기재)

## 예상 오류 원인
(오류수정 TASK에서만 필수. 코드 분석을 기반으로 각 이슈별 예상 원인을 기술한다.)
(기능개선/신규기능 TASK에서는 이 섹션을 생략한다.)
...
### 1.1 작업 범위
(이 TASK가 무엇을 하는지 2~3문장으로 기술. 사람이 읽고 작업 스코프를 이해할 수 있어야 함)

### 1.2 산출물 목록
(생성/수정할 파일 또는 API 엔드포인트 테이블. 사람이 "이 TASK에서 뭐가 나오는가"를 한눈에 파악)
...
```bash
# 사람이 직접 실행하여 결과를 확인할 수 있는 검증 명령어
# 1. 빌드 확인
bun run build
...
| Step | 목적 | 핵심 원칙 |
|------|------|-----------|
| **Step 1 — 계획서** | 작업 전 사람 검토용 | 작업 범위, 산출물, API 테이블 등으로 "무엇을 만드는가"를 명확히 기술. 사람이 읽고 승인/수정할 수 있어야 함 |
| **Step 2 — 체크리스트** | 작업 중 진행도 추적용 | 소분류별 `- [ ]` 항목으로 구성. builder가 작업 완료 시 체크, 사람이 진행 상황을 한눈에 파악 가능 |
| **Step 3 — 완료 검증** | 작업 후 결과 확인용 | 실행 가능한 bash 명령어 블록. 사람이 복사-붙여넣기로 검증을 재현할 수 있어야 함 |
...
## 2. 오류 원인 및 수정 방법
(오류수정 TASK에서만 필수. 각 이슈별로 실제 원인과 수정 방법을 기술한다.)
(기능개선/신규기능 TASK에서는 이 섹션을 생략한다.)
...
## 5. 발견 이슈 및 수정 내역
(작업 중 추가로 발생한 오류·수정 사항을 이슈별로 기술)
