...
export default function LinkedTasksPanel({
  ...
  if (!projectId) {
    return (
      <div
        className="flex flex-col gap-3 p-3 border-l border-[var(--gray-border)] bg-white"
        style={{ minWidth: '220px' }}
      >
        <p className="text-[12px] text-[var(--text-sub)]">이 항목은 프로젝트가 지정되지 않았습니다.</p>
      </div>
    );
  }
  ...
  return (
    <div
      className="flex flex-col gap-3 p-3 border-l border-[var(--gray-border)] bg-white overflow-y-auto"
      style={{ minWidth: '220px', maxHeight: '400px' }}
    >
      {/* 탭 */}
      <div className="flex gap-1 border-b border-[var(--gray-border)]">
      ...
      {/* 액션 버튼 */}
      {activeTasks.length > 0 && (
        <div className="flex flex-col gap-1.5 border-t border-[var(--gray-border)] pt-2">
          <Button
            size="small"
            variant="ghost"
